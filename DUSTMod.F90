module DUSTMod
  ...
  use domainMod            , only : ldomain    !dmleung added 2 Aug 2022
  !  
  ! !PUBLIC TYPES
  implicit none
  private
  ...
  type, public :: dust_type
     ...
     real(r8), pointer, private :: u_mean_slt_patch          (:)   ! translation of mean friction velocity to 0.1 m level of dust saltation (m/s)
     real(r8), pointer, private :: u_sd_slt_patch            (:)   ! translation of sd of friction velocity to 0.1 m level of dust saltation (m/s)
     real(r8), pointer, private :: u_fld_thr_patch           (:)   ! translation of fluid threshold to 0.1 m level of dust saltation (m/s)
     real(r8), pointer, private :: u_impct_thr_patch         (:)   ! translation of impact threshold to 0.1 m level of dust saltation (m/s)
     real(r8), pointer, private :: thr_crs_rate_patch        (:)   ! threshold crossing rate (unitless)
     real(r8), pointer, private :: prb_crs_fld_thr_patch     (:)   ! probability of crossing fluid threshold
     real(r8), pointer, private :: prb_crs_impct_thr_patch   (:)   ! probability of crossing impact threshold
     ...
     real(r8), pointer, private :: ssr_patch                 (:)   ! [dimless] integrated shear stress ratiio, defined by Okin (2008) and then integrated by Pierre et al. (2014)
     ...
     real(r8), pointer, private :: frc_thr_rghn_fct_patch    (:)   ! [dimless] roughness factor
   contains
...
contains
  ...
  subroutine DustEmission (bounds, &
    ...
    integer  :: index
    !########### added by dmleung 27 Nov 2021 #########################################
    real(r8) :: tmp2   ! added as the value couldn't pass from Dustini to DustEmission; will be calculated here  23 May 2020 -dml
    ...
    real(r8) :: wnd_frc_thr_slt_it  ! [m/s] created for impact threshold friction velocity, dml 9 Jun 2021
    real(r8) :: wnd_frc_thr_slt     ! [m/s] used for wet fluid threshold friction velocity, dml 9 Jun 2021
    !########### added by dmleung 20 Dec 2021 for drag partition effect #################
    real(r8) :: K_length            ! [dimless] normalized mean interobstacle distance, or called gap length 
    !    
    ! constants
    !
    ...
    !####### added by dmleung 27 Nov 2021 ###########################################################################
    real(r8), parameter :: vai_mbl_thr = 1.0_r8        ! [m2 m-2] new VAI threshold, -dmleung 27 Nov 2021
    ...
    !####### added by dmleung 2 Dec 2021 for intermittency ##########################################################
    real(r8), parameter :: U_0 = 0.32_r8               ! [dimless] SSR in the immediate lee of a plant, dimensionless
    ...
    associate(                                                         & 
         ...
         wnd_frc_thr         => dust_inst%wnd_frc_thr_patch          , & ! output impact threshold -dml
      ...
      do fp = 1,num_nolakep
         ...
         ! output moisture variables -dml, coded Jul 2020, recoded 18 Mar 2021, added to CLM5 27 Nov 2021
         ...
         ! use tmp1 from DUSTini for I&W (1982) (75 um is optimal); use tmp3 for S&L (2000) (107 um is optimal)
         ! recoded to CLM5 27 Nov 2021
         !#######################################################################################################

         tmp2 = 1.0_r8*sqrt(0.0123_r8 * (dns_slt*grav*130.0e-6_r8 + 1.65e-4_r8/130.0e-6_r8)) ! calculate S&L (2000) scheme here for threshold
         ...
         !wnd_frc_thr_slt_it = B_it * tmp1 / sqrt(forc_rho(c)) ! define impact threshold -dml 9 Jun 2021, recoded to CLM5 27 Nov 2021
         ! the above formula is true for Iversen and White (1982) and Shao and Lu (2000) scheme -dml, 23 Feb 2020, added to CLM5 27 Nov 2021
         wnd_frc_thr(p) = wnd_frc_thr_slt          ! output fluid threshold -dml
         ...
         if (lnd_frc_mbl(p) > 0.0_r8  .AND. tlai_lu(l)<=1_r8) then
            ...
            ssr(p) = (K_length+U_0*c_e)/(K_length+c_e)
            ...
            wnd_frc_slt = fv(p) * frc_thr_rgh_fct   ! wnd_frc_slt will be used in dust emission equation  -dmleung
         ...
         if (lnd_frc_mbl(p) > 0.0_r8) then
            ...
            if (wnd_frc_slt > wnd_frc_thr_slt_it) then! if want to use impact threshold for dust emission, uncomment this one, -dmleung 2 Dec 2021
               ...
               !flx_mss_vrt_dst_ttl(p) = flx_mss_vrt_dst_ttl(p) * lnd_frc_mbl(p) * C_tune ! dmleung took away liqfrac

               if (ldomain%frac(g) < 0.5_r8) then
               flx_mss_vrt_dst_ttl(p) = flx_mss_vrt_dst_ttl(p) * 1e-3_r8 !  discard coastal and islandic emissions
               end if
               !########################################################################################
            end if
            ...
            if (intrmtncy_fct(p) /= intrmtncy_fct(p)) then  ! if intrmtncy_fct(p) is not NaN then multiply by intermittency factor; this statement is needed because dust emission flx_mss_vrt_dst_ttl(p) has to be non NaN (at least zero), dml 9 Jun 2021
