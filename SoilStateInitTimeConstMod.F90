...
contains
  ...
  subroutine SoilStateInitTimeConst(bounds, soilstate_inst, nlfilename) 
    ...
    use clm_varctl          , only : iulog, fsurdat, paramfile, soil_layerstruct, &
                                     clay_frc, &            ! -dmleung added clay_frc, 2 Mar 2021, re-added to CESM2/CLM5 14 Dec 2021
                                     rough_fct, lulc_frc    ! -dmleung re-added to CESM2/CLM5 17 Dec 2021
    ...
    real(r8) ,pointer  :: clayfrc2d(:)                  ! read in - clay percentage (called fraction here) from SoilGrids, -dml, 2 Mar 2021, dmleung re-added 14 Dec 2021
    real(r8) ,pointer  :: roughfct2d(:)                 ! read in - time-invariant roughness factor   -dml, 16 Jul 2020, dmleung re-added 17 Dec 2021
    real(r8) ,pointer  :: rockfrc2d(:)                  ! read in - rock area fraction from GLCNMO -dml, 7 Jul 2021, dmleung re-added 17 Dec 2021
    real(r8) ,pointer  :: vegefrc2d(:)                  ! read in - shrub/crop area fraction from GLCNMO -dml, 7 Jul 2021, dmleung re-added 17 Dec 2021
    real(r8) ,pointer  :: sparfrc2d(:)                  ! read in - sparse vegetation area fraction from GLCNMO -dml, 7 Jul 2021, dmleung re-added 17 Dec 2021
...
! remove roots below bedrock layer
    ...
    ! dmleung re-added to CESM2/CLM5 14 Dec 2021
    allocate(clayfrc2d(begg:endg))                                    ! dml, 2 Mar 2021
    ! read SoilGrids clay fraction, dml, 2 Mar 2021
    ...
    ! dmleung re-added to CESM2/CLM5 17 Dec 2021
    allocate(roughfct2d(begg:endg))                                   ! dml, 16 Jul 2020
    ! here to read roughness factor file, dml, 16 Jul 2020
    write(iulog,*) 'Attempting to read roughness factor data, by dml .....'
    ...
    write(iulog,*) 'initialize pft level roughness factor from roughfct2d(g) to roughfct(p)'   ! -dml
    ...
    ! dmleung re-added to CESM2/CLM5 17 Dec 2021
    allocate(rockfrc2d(begg:endg));allocate(vegefrc2d(begg:endg));allocate(sparfrc2d(begg:endg)) ! dml, 7 Jul 2021
    write(iulog,*) 'Attempting to read GLCNMO LULC area fraction data, by dml .....'
    ...
    do c = begc,endc
       g = col%gridcell(c)
       !!!## if want to use default CESM (FAO) clay for dust emission's moisture effect, uncomment below ##
       !!!!!soilstate_inst%gwc_thr_col(c) = 0.17_r8 + 0.14_r8 * clay3d(g,1) * 0.01_r8  !- dmleung commented, 27 Nov 2021
       soilstate_inst%gwc_thr_col(c) = 0.01_r8*2_r8*(0.17_r8*clay3d(g,1) + 0.0014_r8*clay3d(g,1)*clay3d(g,1))  !Fecan et al. (1999) -jfk, dmleung coded 27 Nov 2021
       soilstate_inst%mss_frc_cly_vld_col(c) = min(clay3d(g,1) * 0.01_r8, 0.20_r8)

       !!!## if want to use alternative clay data (SoilGrids) for dust emission's moisture effect, uncomment below ##
       !soilstate_inst%gwc_thr_col(c) = 0.01_r8*(0.17_r8*clayfrc2d(g) + 0.0014_r8*clayfrc2d(g)*clayfrc2d(g))  !Fecan et al. (1999) -jfk, dmleung coded 14 Dec 2021
       !soilstate_inst%mss_frc_cly_vld_col(c) = min(clayfrc2d(g) * 0.01_r8, 0.20_r8) !-dmleung 14 Dec 2021
    end do
